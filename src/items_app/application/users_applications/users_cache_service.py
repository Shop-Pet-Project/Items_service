...
class UserCacheService:
    ...
    async def set_user(self, user: User) -> None:
        ...
        cache_key = self._cache.generate_key("user", user.id)
    ...
    async def mset_users(self, users: List[User]) -> None:
        ...
        if not users:
            return
        cache_mapping = {
            self._cache.generate_key("user", user.id): user for user in users
        }
        await self._cache.mset(**cache_mapping)

    async def set_all_users_list(self, users: List[User], offset: int, limit: int) -> None:
    ...
    async def mget_users(self, user_ids: List[UUID]) -> List[Optional[User]]:
    ...
    async def invalidate_on_delete(self, user_id: UUID):
        ...
        await self.delete_user(user_id) # Удаляем кеш конкретного юзера
        await self.invalidate_all_users_list() # Инвалидируем списки