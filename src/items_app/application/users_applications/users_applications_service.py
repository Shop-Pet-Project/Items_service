...
class UsersApplicationsService:
    ...
    async def add_user(self, user_data: User) -> Optional[User]:
        ...
        try:
            ...
            existing_user_by_username = self._user_repo.get_user_by_username(user_data.username)
    ...
    async def get_user_by_id(self, user_id: UUID, current_user: User) -> Optional[User]:
        ...
        try:
            ...
            if current_user.role != 'admin' and current_user.id != user_id:
                raise AccessToAnotherUserError(user_id)
            
            # 2. Попытка получить данные из кеша
            if cache_value := await self._cache.get_user(user_id):
                return cache_value
    ...
    async def get_multiple_users_by_ids(
        ...
        try:
            ...
            cached_users_data = await self._cache.mget_users(unique_user_ids)
            ...
            ids_to_fetch_from_db = [uid for uid in unique_user_ids if uid not in found_user_ids]

            # 5. Если остались ID для поиска, идем в базу
            if ids_to_fetch_from_db:
                db_users = await self._user_repo.get_several_users_by_ids(ids_to_fetch_from_db)
            ...
            missing_user_ids = [uid for uid in unique_user_ids if uid not in final_found_ids]

            # 8. Возврат результата
            return MultipleUsersResponse(found_users=found_users, missing_user_ids=missing_user_ids)
        except SQLAlchemyError as e:
    ...
    async def update_user(self, update_data: User, current_user: User) -> Optional[User]:
        ...
        try:
            ...
            if current_user.role != 'admin' and current_user.id != update_data.id:
                raise AccessToAnotherUserError(update_data.id)
            ...
            if update_data.username != existing_user.username:
                user_with_same_username = await self._user_repo.get_user_by_username(update_data.username)
                if user_with_same_username:
                    raise UsernameAlreadyExistError(update_data.username)
            
            if update_data.email != existing_user.email:
                user_with_same_email = await self._user_repo.get_user_by_email(update_data.email)
